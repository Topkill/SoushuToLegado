...
另外，搜书大师备份本身也**不能默认包含**用户导入的本地 txt/epub 正文：实测样本里额外打进包的主要是 `.wbpub` 网文缓存；本地 txt 即使在书架上也可能不在 `_names.list` 中。

目标 `legado-E` 官方备份同样不会打包本地书原文件。因此转换后的目标 zip 通常只能保存书籍记录中的路径，不能指望官方恢复流程把本地书正文一起恢复到设备。

如果后续要完整迁移本地书正文，有两个选择：

1. 用户自行拷贝本地 txt/epub 到目标设备固定路径，并保证与书架记录路径一致或重新导入。
2. 做一个专用导入工具/改 App 恢复逻辑，额外携带并写回文件型书籍。

第一版建议只迁移书架记录和可用元数据；本地书正文是否存在于搜书大师备份，要以实际 `_names.list` 为准，不要想当然。
...
- 本地书/外部书文件打包（样本显示多为 .wbpub；本地 txt 不保证在备份内）。
- 章节目录。
- 正文缓存。
- Cookie。
- 源 App 配置。
