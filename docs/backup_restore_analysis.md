...
本地备份的核心行为是：把应用私有数据目录打成一个 zip 包，并额外把书架和最近阅读中的**本机书籍文件**（主要是用户导入的**本地书**，如 txt；以及这些书的伴随元数据）也打进同一个包。这里说的本机书籍文件，指设备上有文件路径、需要随备份带走的**本地书**文件，不是指“应用私有目录里的配置数据”。备份文件默认落在外部存储的 `Books/Soushu/ssds.backup`，恢复时会解包覆盖到当前应用数据目录，并对部分设备相关配置做适配。
...
- `sources/com/flyersoft/components/t.java`
  ...
  - `h(...)`：递归打包内部数据目录
  - `w(ZipOutputStream)`：额外打包本地书文件和伴随元数据
  - `r(String path)`：备份过滤器
...
它会收集两类本机书籍文件（路径指向设备上的书文件，通常是本地书）：
