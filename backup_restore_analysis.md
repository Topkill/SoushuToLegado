...
本地备份的核心行为是：把应用私有数据目录打成一个 zip 包，并额外把书架和最近阅读中的本地书籍文件及其伴随元数据也打进同一个包。备份文件默认落在外部存储的 `Books/Soushu/ssds.backup`，恢复时会解包覆盖到当前应用数据目录，并对部分设备相关配置做适配。
...
- `sources/com/flyersoft/components/t.java`
  ...
  - `h(...)`：递归打包内部数据目录
  - `w(ZipOutputStream)`：额外打包本地书籍文件和伴随元数据
  - `r(String path)`：备份过滤器
...
源 APK 的书架和书籍详情关系不在单独文件里，也不在 `_names.list` 里。`_names.list` 只负责把 `1.tag`、`2.tag` 这类文件名映射回真实路径。
...
3. 本地备份默认会带上本地书籍原文件，备份体积可能很大。
4. 外部书籍文件路径也会进入 `_names.list`，恢复时可能尝试写回原路径。
